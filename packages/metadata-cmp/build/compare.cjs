...
// Copyright 2018-2021 @axia-js/metadata-cmp authors & contributors
// SPDX-License-Identifier: Apache-2.0
const [ws1, ws2] = _yargs.default.demandCommand(2).argv._;
...
function log(pad, title, pre, text, post) {
  console.log(createLog(pad, title, pre, text, post));
}

function createLog(pad, title, pre, text, post) {
...
function logArray(pad, title, pre, arr, chunkSize) {
  if (arr.length) {
    let first = true;
...
async function getMetadata(url) {
  (0, _util.assert)(url.startsWith('ws://') || url.startsWith('wss://'), `Invalid WebSocket endpoint ${url}, expected ws:// or wss://`);
  const provider = new _api.WsProvider(url);
...
async function main() {
  ...
  const b = metaB.asLatest; // configure padding
  ...
  const mA = a.modules.map(({
    name
  }) => name.toString());
  const mB = b.modules.map(({
    name
  }) => name.toString());
  ...
  console.log();
  const decA = (0, _types.expandMetadata)(metaA.registry, metaA);
  const decB = (0, _types.expandMetadata)(metaB.registry, metaB);
  mA.filter(m => mB.includes(m)).forEach(m => {
    var _decA$tx$n$eA$, _decB$tx$n$eB$;

    const n = (0, _util.stringCamelCase)(m);
    ...
    const index = createCompare((_decA$tx$n$eA$ = decA.tx[n][eA[0]]) === null || _decA$tx$n$eA$ === void 0 ? void 0 : _decA$tx$n$eA$.callIndex[0], (_decB$tx$n$eB$ = decB.tx[n][eB[0]]) === null || _decB$tx$n$eB$ === void 0 ? void 0 : _decB$tx$n$eB$.callIndex[0]);
    ...
    eA.filter(c => eB.includes(c)).forEach(c => {
      const cA = decA.tx[n][c];
      const cB = decB.tx[n][c];
      const tA = cA.meta.args.map(({
        type
      }) => type.toString());
      const tB = cB.meta.args.map(({
        type
      }) => type.toString());
    ...
    sA.filter(c => sB.includes(c)).forEach(c => {
      const cA = decA.query[n][c];
      const cB = decB.query[n][c]; // storage types differ
